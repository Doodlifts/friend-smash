// @ts-nocheck
/* ============================================================
   components/engine.ts — FRIEND SMASH game engine.

   Imperative canvas engine, structured for Next.js/React:

     • Piece art + wordmark are generated from Rare Friends portraits
       (lib/rf/pieceArt).
...
export function startEngine(): () => void {
  ...
  // Accessibility: honor the OS "reduce motion" setting — keep the pixel
  // shatter but skip the full-screen impact flash + screen shake.
  ...
  const tuning = { fx: { intensity: 1 }, scoring: { v: SCORING_ALGO_V } };
  function applyPublicConfig(cfg){
    try{
      if (cfg && cfg.fx && typeof cfg.fx.intensity === "number")
        tuning.fx.intensity = Math.min(2, Math.max(.25, cfg.fx.intensity));
  ...
  // menu Friends bob from art[] — rebuilt at boot AND on every reconcile
  // (the menu is where players sit between games; it must pick up the
  // wallet's Friends as soon as they load).
  ...
  const G = {
    ...
    state:"menu",          // menu | play | clearing | paused | over
  ...
  let activeRun = null;    // { seed, runId, runToken } from window.__RFSMASH_RUN
  ...
  function usePowerup(key){
    ...
    if (window.__RFSMASH_RUN && typeof window.__RFSMASH_RUN.consume === "function"){
      try{ window.__RFSMASH_RUN.consume({ runId:activeRun.runId, runToken:activeRun.runToken, key, n:puUses[key] }); }catch(e){}
    }
    switch(key){
      case "slow_fall": slowUntil = performance.now() + 15000; break;
      case "next_peek": previewN = 5; drawSide(); break;
      case "clean_slate": Shatter.reset(); break;
  ...
  function lockPiece(){
    ...
    if (bombArmed){
      ...
      Shatter.add([Math.max(HIDDEN, Math.min(TOTAL-1, p.y+1))]); impactFlash([p.y+1]);
    }
    const rows = [];
    ...
    if (rows.length){
      // every Friend touching the smashed rows switches to its flash art
      const hit = new Set();
      for (const y of rows) for (const c of G.grid[y]) hit.add(c.id);
      for (const i of hit) G.inst[i].v = "flash";
      Shatter.add(rows);
      impactFlash(rows);
      // SMASH celebration fires WITH the shatter, not after it (playtest: it
      // used to land in finishClear, ~0.5s late — after the shatter had
      // already passed). G.smashArmed is this lock's flag; scoring
  ...
  function finishClear(){
    ...
    // pixel particles along cleared rows
    ...
    updHud();
    G.clearing = null;
    // (the SMASH burst fires at clearing START, with the shatter — see lockPiece)
    G.state = "play";
    spawn();
  }
  ...
  function gameOver(){
    ...
    if (isBest){ G.best = G.score; try{ localStorage.setItem("rfsmashBest", String(G.best)); }catch(e){} }
    ...
    if (recorder.active && activeRun && window.__RFSMASH_RUN && typeof window.__RFSMASH_RUN.finish==="function"){
      const summary = {
      ...
      try{ window.__RFSMASH_RUN.finish({ runId:activeRun.runId, runToken:activeRun.runToken, seed:activeRun.seed, summary, log:recorder.log.slice(), clientScore:G.score }); }catch(e){}
    }
  ...
  /* ============================================================ SHATTER
     Cosmetic clear effects: pixel shards + a brief impact flash. Cleared
     on restart. Never touches scoring or replay.                        */
  // SMASH-clear celebration bursts: pushed by lockPiece at clearing START
  // (with the shatter — playtest: firing from finishClear landed late),
  // drawn in drawBoard, self-expiring.
  // Pure presentation — replay/scoring untouched.
  const smashFX = [];
  // Pixel "SMASH!" burst wordmark.
  const smashWord = new Image();
  try{ smashWord.src = wordmarkDataUrl(["SMASH!"], 8); }catch(_){ }

  const Shatter = {
    shards: [],   // canvas-space square pixel shards arcing up and off the board
    lastT: 0,     // last update timestamp (for frame-independent motion)
    add(rows){
      ...
      // off the bottom of the board. Cosmetic only.
      const I = tuning.fx.intensity;
      for (const y of rows){
        const row = (G.grid && G.grid[y]) || [];
        for (let x=0;x<COLS;x++){
          const cell = row[x];
          const ins = cell && G.inst ? G.inst[cell.id] : null;
          const col = ins && PIECE_STYLE[ins.t] ? PIECE_STYLE[ins.t].color : ACCENT;
      ...
      if (this.shards.length>220) this.shards.splice(0, this.shards.length-220);
    },
    reset(){
      this.shards = []; this.lastT = 0;
    },
  };
  ...
  /* ---------- CAMERA WORLD: camera backdrop, one branded opt-in ----------
     The "let your Friends into your world?" card is the ONLY place the
     camera prompt is triggered for first-timers (a tap = the gesture
     browsers require). Prefs persist: rfsmashWorld (asked?), rfsmashCam.
     ...
     settles. Everything fails open to the pastel sky. The stream never
     leaves the device. */
  let camStream = null;
  ...
  function updateWorldUi(){
    ...
    if (chip) chip.classList.toggle("hidden", !!camStream || !camSupported() || pref("rfsmashCam") === "on");
  }
  ...
  function drawBoard(){
    ...
    if (Shatter.shards.length){
      const tnow = performance.now();
      const gdt = Math.min(40, tnow - (Shatter.lastT || tnow)) / 1000;
      Shatter.lastT = tnow;
      const GRAV = CELL*9;
      const EXIT = (TOTAL+1)*CELL;
      for (let i=Shatter.shards.length-1;i>=0;i--){
        const g = Shatter.shards[i];
        g.vy += GRAV*gdt; g.x += g.vx*gdt; g.y += g.vy*gdt; g.vx *= .99;
        g.rot += g.vr*gdt;
        if (g.y > EXIT){ Shatter.shards.splice(i,1); continue; }
    ...
    // ----- SMASH-clear celebration: pixel "SMASH!" burst over the cleared band -----
  ...
  function startGame(){
    ...
    // CAMERA WORLD gate: first-timers see the branded ask only AFTER tapping
    ...
    // starts the game they asked for. Answering sets the rfsmashWorld pref,
    // so this fires at most once ever.
    try{
      const isTouch = window.matchMedia && window.matchMedia("(pointer:coarse)").matches;
      if (false && !pref("rfsmashWorld") && isTouch && camSupported()){ // camera world is opt-in from the menu chip only
    ...
    try{
      if (pref("rfsmashCam") === "on" && !camStream && camSupported()){
        ...
        if (!(camHoldSpent && camPending)){
          ...
          const holdAcquire = ()=>{
            startCamera().then((ok)=>{
              ...
              if (!ok && camLastError === "NotAllowedError"){ setPref("rfsmashCam", "off"); updateWorldUi(); }
          ...
          // fronts it — SMASH! → "let your Friends into your world?" → the
          // device prompt rides that YES. Remembered grants stay silent.
          queryCamPermission().then((state)=>{
            if (disposed){ release(); return; }
            if (state === "granted") return holdAcquire();
            release();
            if (state === "denied") return beginRun(); // browser-blocked: sky, no nagging
            if (camCardShown) return beginRun(); // card already fronted this round — sky
            camCardShown = true;
            worldPendingFn = startGame;
            showWorldAsk("resume"); // its YES tap carries the camera ask
          });
          return;
        }
      }
  ...
  function beginRun(){
    ...
    if (recorder.active && activeRun && window.__RFSMASH_RUN && typeof window.__RFSMASH_RUN.abandon === "function"){
      try{ window.__RFSMASH_RUN.abandon({ runId:activeRun.runId, runToken:activeRun.runToken, log:recorder.log.slice() }); }catch(e){}
    }
    // Pull a server-sanctioned run (seed + token) if one was prefetched; else
    // play a local, unranked game with a fresh random seed.
    let run = null;
    try{ run = (window.__RFSMASH_RUN && typeof window.__RFSMASH_RUN.takeRun==="function") ? window.__RFSMASH_RUN.takeRun() : null; }catch(e){ run = null; }
    activeRun = run;
  ...
  /* --- CAMERA WORLD wiring: branded ask → device prompt → easy toggle ----
  ...
  const wireOnce = (el, fn)=>{
    if (!el || el.dataset.rfsmashWired) return;
    el.dataset.rfsmashWired = "1";
  ...
  let worldMode = "first";
  // Short, but says exactly what YES will ask for and why — the camera
  // pop-up lands right after this card, so no surprises.
  ...
  wireOnce(document.getElementById("btnWorldYes"), ()=>{
    const firstAnswer = !pref("rfsmashWorld");
    setPref("rfsmashWorld", "asked");
    startCamera().then((ok)=>{
      ...
      if (ok) setPref("rfsmashCam", "on");
      else if (firstAnswer || camLastError === "NotAllowedError") setPref("rfsmashCam", "off");
      updateWorldUi();
  ...
  wireOnce(document.getElementById("btnWorldNo"), ()=>{
    ...
    //   first answer  → record the opt-out;
    ...
    if (!pref("rfsmashWorld")){
      setPref("rfsmashCam", "off");
  ...
  wireOnce(document.getElementById("btnWorldOpen"), ()=>showWorldAsk(pref("rfsmashWorld") ? "chip" : "first"));
  ...
  G.vs = null;
  let vsDriver = 0;
  const MATCH = ()=> (typeof window !== "undefined" ? window.__RFSMASH_MATCH : null) || null;
  ...
  function vsStartRound(m){
    ...
    hideAll();
    if (m.config) applyPublicConfig(m.config); // the match's shared tuning snapshot
    G.vs.phase = "live";
  ...
  function vsTick(){
    ...
    if (L.state === "over" && L.result){
      if (turfFxActive()) return; // let the row-win FX finish first
      const r0 = L.result;
      if (r0.id && r0.id === vsAckedResult){ vsExit(); return; } // already dismissed
      // Match ended on THEIR pure row: play the row-win FX on the final board
      // (turfFinal) once before the verdict card. Winners already saw it via
      // their own place.
      if (!r0.won && r0.turfFinal && r0.turfFinal.row != null && turfc &&
          turfFx.key !== "over:" + (r0.id || "")){
        turfRunRowFX(r0.turfFinal.placements, r0.turfFinal.row, "over:" + (r0.id || ""), null);
        return; // the card renders on the first tick after the FX
      }
  ...
  // it. Wallet-session auth can resolve slowly on cold mobile loads, so retry the
  ...
  function vsStaleReload(){
    try{
      if (sessionStorage.getItem("rfsmashStaleReload")) return; // no reload loops
      sessionStorage.setItem("rfsmashStaleReload", "1");
    }catch(_){ }
    location.reload();
  }
  ...
  /* Row-win FX: the winning row's Friends flip to their flash variants, the
     row strobes and bursts into pixel shards for ~1.6s BEFORE any
     splash/verdict. turfFx.until holds turfSync and the over-card back while
     the canvas is mid-FX. */
  const turfFx = { key: "", until: 0 };
  function turfFxActive(){ return Date.now() < turfFx.until; }
  function turfRunRowFX(placements, row, key, done){
    ...
    const DUR = 1600;
    // Shard geometry in CELL units — cpx is re-read every frame so a mid-FX
    ...
    for (let c = 0; c < TURF_COLS; c++){
      for (let k = 0; k < 3; k++){
        shards.push({
          ...
          vx: (Math.random() - .5)*3,
          vy: -(2 + Math.random()*4),
          r: Math.random() < .5 ? .16 : .26,
          col: Math.random() < .45 ? INK : ACCENT,
        });
      }
    }
    ...
    const step = ()=>{
      ...
      for (const p of placements){
        turfDrawPiece(x2, p.t, p.r, p.x, p.y, p.team, null, inRow(p) ? "flash" : "clean");
      }
      // ink/white strobe over the winning row (same beat as the solo clear flash)
      if (!reduceMotion || el < .5){
        const a = (1 - el)*(.25 + .45*Math.abs(Math.sin(el*Math.PI*6)));
        x2.fillStyle = "rgba(255,255,255," + a.toFixed(3) + ")";
        x2.fillRect(0, row*cpx, TURF_COLS*cpx, cpx);
      }
      // pixel shards arcing up out of the row and falling away
      if (!reduceMotion){
        const ts = el*DUR/1000;
        for (const d of shards){
          const sx = (d.x + d.vx*ts)*cpx;
          const sy = (row + .5 + d.vy*ts + 4.5*ts*ts)*cpx;
          const R = Math.max(2, d.r*cpx);
          x2.fillStyle = INK;
          x2.fillRect(Math.round(sx - R/2) - 2, Math.round(sy - R/2) - 2, Math.round(R) + 4, Math.round(R) + 4);
          x2.fillStyle = d.col;
          x2.fillRect(Math.round(sx - R/2), Math.round(sy - R/2), Math.round(R), Math.round(R));
        }
      }
  ...
  function turfSync(L){
    ...
    if (turfFxActive()) return; // the board is mid-FX — sync resumes after
    // Opponent finished a row: play the row FX on THEIR winning board (carried in
    ...
    if (turfc && turfc.game !== T.game && T.lastGame && T.lastGame.reason === "row" &&
      ...
      const lg = T.lastGame;
      turfRunRowFX(lg.placements, lg.row, "game:" + T.id + ":" + lg.n, ()=>{
        if (!turfc) return;
      ...
      return; // keep the OLD board on screen while the FX plays
    }
    const first = !turfc;
    ...
    if (first){
      ...
      if (recorder.active && activeRun && window.__RFSMASH_RUN && typeof window.__RFSMASH_RUN.abandon === "function"){
        try{ window.__RFSMASH_RUN.abandon({ runId:activeRun.runId, runToken:activeRun.runToken, log:recorder.log.slice() }); }catch(_){ }
      }
      recorder.active = false; activeRun = null;
      G.state = "vswait"; G.cur = null;
      Music.stop();
  ...
  (function(){
    const cv2 = turfEl("turfCv");
    if (cv2 && !cv2.dataset.rfsmashWired){
      cv2.dataset.rfsmashWired = "1";
      const aim = (e)=>{
      ...
      M.place({ matchId: turfc.matchId, moveN: turfc.moveN, t: optim.t, r: optim.r, x: optim.x, y: optim.y, rulesV: TURF_RULES_V })
        .then((data)=>{
          ...
          if (data.ok && data.gameOver && turfc){
            ...
            // play the row FX, then celebrate.
            let fxRow = null;
            ...
            if (turfFx.key === wantKey){
              ...
              turfRunRowFX(turfc.placements, fxRow, wantKey, showIt);
            } else {
              showIt();
            }
          }
  ...
  wireOnce(document.getElementById("btnVsFind"), ()=>{
    Sfx.init();
    // CAMERA WORLD rides versus too (tester report: the room never showed in
    // a match). First-timers get the branded card on THIS tap — its YES
    // carries the camera ask, then queues. Returning opt-ins reacquire
  ...
  btnMute.addEventListener("click", ()=>{
    ...
    try{ localStorage.setItem("rfsmashMuted", Sfx.on ? "0" : "1"); }catch(_){}
  ...
  const abandonActiveRun = (beacon)=>{
    if (!(recorder.active && activeRun && window.__RFSMASH_RUN && typeof window.__RFSMASH_RUN.abandon === "function")) return;
    try{
      const log = beacon ? recorder.log.filter(e=>e.a==="powerup") : recorder.log.slice();
      window.__RFSMASH_RUN.abandon({ runId:activeRun.runId, runToken:activeRun.runToken, log, beacon:!!beacon });
    }catch(e){}
    recorder.active = false; activeRun = null;
  };
  ...
  const onPageShow = (e)=>{
    if (e && e.persisted && (G.state==="play" || G.state==="paused" || G.state==="clearing") && !activeRun && recorder && !recorder.active){
      quitGame();
    }
  };
  ...
  /* ============================================================ BOOT */
  try{ G.best = parseInt(localStorage.getItem("rfsmashBest")||"0",10)||0; }catch(e){}
  // restore the persisted sound preference (Sfx.on gates both SFX + music start)
  try{ if (localStorage.getItem("rfsmashMuted")==="1"){ Sfx.on=false; } }catch(e){}
  btnMute.innerHTML = Sfx.on ? SND_ON : SND_OFF; // reflect state with the on-brand glyph
  // CAMERA WORLD boot: deliberately NO camera call here — iOS Safari can
  ...
  return function cleanup(){
    ...
    clearTimeout(settle1); clearTimeout(settle2); clearTimeout(camWatchdog);
